...
# proxy_machine_sg: Allow ONLY web proxy traffic from the proxied subnet

resource "aws_vpc_security_group_ingress_rule" "allow_proxy_explicit" {
...
# except for traffic destined for an IP covered by var.proxied_subnet_escape_routes
...
output "http_proxy_var" {
  ...
  value       = "http://${aws_instance.proxy_machine.private_ip}:8080"
}
output "https_proxy_var" {
  description = "value for HTTPS_PROXY environmental variable"
  value       = "https://${aws_instance.proxy_machine.private_ip}:8080"
}
